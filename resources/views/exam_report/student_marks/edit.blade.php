...
<div class="container">
    <div class="row page-title-row">
        <div class="col-md-12">
            <h3>Student Marks <small>&raquo; Edit Marks</small></h3>
        </div>
    </div>
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            @component('components.panelWithHeading')
                @slot('title')
                    Student Marks Edit Form
                @endslot

               <form class="form-horizontal" method="POST" action="{{ route('student-marks.update', $marks->id) }}">
                    {{ method_field('PATCH') }}
                    {{ csrf_field() }}
                    ...
                    <div class="form-group">
                        <label for="marks" class="col-md-4 control-label">Student Name</label>
                         <div class="col-md-6">
                            <input type="text" 
                                class="form-control" 
                                value="{{ $marks->academicInfo->student->name }}"
                                readonly>
                        </div>
                    </div>
